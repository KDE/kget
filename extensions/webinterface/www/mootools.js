...
var MooTools={version:"1.2dev",build:"1373"};var Native=function(J){J=J||{};var F=J.afterImplement||function(){};var G=J.generics;G=(G!==false);var H=J.legacy;
...
}if((function(){while(K&&K.nodeType==3){K=K.parentNode;}}).create({attempt:Browser.Engine.gecko})()===false){K=false;}}}}return $extend(this,{event:A,type:J,page:I,client:C,rightClick:E,wheel:H,relatedTarget:K,target:G,code:B,key:L,shift:A.shiftKey,control:A.ctrlKey,alt:A.altKey,meta:A.metaKey});
...
},xpath:Selectors.Pseudo.nth.xpath,filter:Selectors.Pseudo.nth.filter}});Element.Events.domready={onAdd:function(A){if(Browser.loaded){A.call(this);}}};
(function(){var A=function(){if(Browser.loaded){return ;}Browser.loaded=true;window.fireEvent("domready");document.fireEvent("domready");};switch(Browser.Engine.name){case"webkit":(function(){(["loaded","complete"].contains(document.readyState))?A():arguments.callee.delay(50);
})();break;case"trident":(function(){var B=document.createElement("div");try{B.doScroll("left");}catch(C){arguments.callee.delay(50);return ;}A();})();
break;default:window.addEvent("load",A);document.addEvent("DOMContentLoaded",A);}})();var JSON=new Hash({encode:function(B){switch($type(B)){case"string":return'"'+B.replace(/[\x00-\x1f\\"]/g,JSON.$replaceChars)+'"';
...
}});return"{"+String(A)+"}";case"number":case"boolean":return String(B);case false:return"null";}return null;},$specialChars:{"\b":"\\b","\t":"\\t","\n":"\\n","\f":"\\f","\r":"\\r",'"':'\\"',"\\":"\\\\"},$replaceChars:function(A){return JSON.$specialChars[A]||"\\u00"+Math.floor(A.charCodeAt()/16).toString(16)+(A.charCodeAt()%16).toString(16);
...
return(A)?A.hsbToRgb():null;}});var Swiff=function(K,L){if(!Swiff.fixed){Swiff.fix();}var J="Swiff_"+Native.UID++;L=$merge({id:J,height:1,width:1,container:null,properties:{},params:{quality:"high",allowScriptAccess:"always",wMode:"transparent",swLiveConnect:true},events:{},vars:{}},L);
var D=L.params,F=L.vars,B=L.id;var G=$extend({height:L.height,width:L.width},L.properties);Swiff.Events[J]={};for(var A in L.events){var E=L.events[A];
Swiff.Events[J][A]=(function(M){return function(){M.call($(L.id));};})(L.events[A]);F[A]="Swiff.Events."+J+"."+A;}D.flashVars=Hash.toQueryString(F);if(Browser.Engine.trident){G.classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000";
D.movie=K;}else{G.type="application/x-shockwave-flash";G.data=K;}var I='<object id="'+L.id+'"';for(var H in G){I+=" "+H+'="'+G[H]+'"';}I+=">";for(var C in D){I+='<param name="'+C+'" value="'+D[C]+'" />';
}I+="</object>";return($(L.container)||new Element("div")).set("html",I).firstChild;};Swiff.extend({Events:{},remote:function(obj,fn){var rs=obj.CallFunction('<invoke name="'+fn+'" returntype="javascript">'+__flash__argumentsToXML(arguments,2)+"</invoke>");
...
}};Element.implement({send:function(A){var B=this.get("send");B.send({data:this,url:A||B.options.url});return this;}});Request.HTML=new Class({Extends:Request,options:{update:false,evalScripts:true,filter:false},processHTML:function(F){var D=F.match(/<body[^>]*>([\s\S]*?)<\/body>/i),A;
F=(D)?D[1]:F;F="<root>"+F+"</root>";if(Browser.Engine.trident){A=new ActiveXObject("Microsoft.XMLDOM");A.async=false;A.loadXML(F);}else{A=new DOMParser().parseFromString(F,"text/xml");
}A=A.getElementsByTagName("root")[0];var C=new Element("div");for(var E=0,B=A.childNodes.length;E<B;E++){var G=Element.clone(A.childNodes[E],true,true);
if(G){C.grab(G);}}return C;},success:function(D){var C=this.options,B=this.response;B.html=D.stripScripts(function(E){B.javascript=E;});var A=this.processHTML(B.html);
B.tree=A.childNodes;B.elements=A.getElements("*");if(C.filter){B.tree=B.elements.filterBy(C.filter);}if(C.update){$(C.update).empty().adopt(B.tree);}if(C.evalScripts){$exec(B.javascript);
}this.onSuccess(B.tree,B.elements,B.html,B.javascript);}});Element.Properties.load={get:function(A){if(A||!this.retrieve("load")){this.set("load",A);}return this.retrieve("load");
},set:function(A){var B=this.retrieve("load");if(B){B.cancel();}return this.store("load",new Request.HTML($extend({link:"cancel",update:this,method:"get"},A)));
...
}}});Element.implement({makeResizable:function(A){return new Drag(this,$merge({modifiers:{x:"width",y:"height"}},A));}});Drag.Move=new Class({Extends:Drag,options:{droppables:[],container:false},initialize:function(D,C){arguments.callee.parent(D,C);
...
var B=this.element.getStyles("left","top");if(B.left=="auto"||B.top=="auto"){this.element.position(this.element.getRelativePosition());}this.element.setStyle("position",A);
...
}return"["+C+"]";},filter:function(G,H){H.i=H.i||0;H.all=H.all||this.parentNode.childNodes;H.len=H.len||H.all.length;var E=H.i;var C=H.len;var F=H.all;
var B=false;var D=G.a+((G.a<0)?C:0);var A=G.b+((G.b<0)?C:0);switch(G.special){case"-":A=(D-A)%C;B=(A<0)?(E<=D||E>=(A+C)):(E<=D&&E>=A);break;case"+":A=(A+D)%C;
case":":B=(A<D)?(E>=D||E<=A):(E>=D&&E<=A);break;default:B=(F[D]==this);}H.i++;return B;}};Hash.Cookie=new Class({Extends:Cookie,options:{autoSave:true},initialize:function(B,A){this.parent(B,A);
