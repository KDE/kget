...
RangeTreeWidget::RangeTreeWidget(QWidget *parent)
    : QTreeView(parent)
    , m_data()
    , m_ranges()
...
QStandardItem *RangeTreeWidget::currentItem(int column)
{
    ...
    if (column >= 0) {
        item =
            m_model->itemFromIndex(m_model->index(m_proxyModel->mapToSource(currentIndex()).row(), column, m_proxyModel->mapToSource(currentIndex()).parent()));
    } else {
...
HostRangeDelegate::HostRangeDelegate(QObject *parent)
    : RangeDelegate(parent)
{
}
...
void RangeTreeWidgetItemDelegate::paint(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const
{
    if (index.parent().isValid()) {
        ...
        painter->drawText(option.rect.left() + 10,
                          ...
                          15,
                          Qt::AlignLeft,
                          item->data(Qt::DisplayRole).toString() + " (" + QString::number(model->rowCount(index)) + ')');
        painter->restore();
        ...
        QRect lineRect(option.rect.left() + 10, option.rect.bottom() - 2, 500, 1);

        QLinearGradient gradient(option.rect.left() + 10, option.rect.top(), 500, option.rect.height());
