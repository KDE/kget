...
//-----------------------------------------------------------------------------
QString
kFileToString(const QString & aFileName, bool aEnsureNL, bool aVerbose)
{
        QCString result;
        ...
        if (!info.isReadable()) {
                if (aVerbose)
                        KMessageBox::error(kmain,
                                           i18n("You do not have read permissions "
                                                "to the file:\n%1").arg(aFileName));
                return QString::null;
        }
        if (len <= 0)
        ...
        if (readLen < len) {
                QString msg =
                        i18n("Could only read %1 bytes of %2.").arg(KGlobal::locale()->
                                        formatNumber
                                        (readLen,
                                         0)).
                        arg(KGlobal::locale()->formatNumber(len, 0));
...
//-----------------------------------------------------------------------------
static bool
kBytesToFile(const char *aBuffer, int len,
             ...
             bool aAskIfExists, bool aBackup, bool aVerbose)
{
        ...
        if (info.exists()) {
                ...
                if (aBackup) {
                        ...
                        rc = rename(QFile::encodeName(aFileName),
                                    QFile::encodeName(bakName));
                        if (rc) {
                                // failed to rename file
                                if (!aVerbose)
                                        return FALSE;
                                rc = KMessageBox::warningYesNo(kmain,
                                                               i18n
        ...
        if (writeLen < 0) {
                ...
                QString msg =
                        i18n("Could only write %1 bytes of %2.").
                        arg(KGlobal::locale()->formatNumber(writeLen,
                                                            0)).arg(KGlobal::locale()->
                                                                    formatNumber(len,
                                                                                 0));
                KMessageBox::error(kmain, msg);
                return FALSE;
...
bool
kCStringToFile(const QCString & aBuffer, const QString & aFileName,
               bool aAskIfExists, bool aBackup, bool aVerbose)
{
...
bool
kByteArrayToFile(const QByteArray & aBuffer,
                 const QString & aFileName, bool aAskIfExists,
                 bool aBackup, bool aVerbose)
{
