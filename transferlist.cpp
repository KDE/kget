...
TransferList::TransferList(QWidget * parent,
                           const char *name):QListView(parent, name)
{
        ...
        lv_resume = addColumn(i18n("Resume"));
        ...
        if (animConn->count() == 0) {
                ...
                for (int i = 0; i < 7; i++) {
                        ...
                        if (p1->isNull()) {
                                KMessageBox::sorry(kmain,
                                                   i18n("Could not load icon\n") +
                                                   iconName, i18n("Error"));
                        }
...
Transfer *TransferList::addTransfer(const KURL & _source,
                                    const KURL & _dest)
{
...
void TransferList::moveToBegin(Transfer * item)
{
        ...
        Transfer *new_item =
                new Transfer(this, item->getSrc(), item->getDest());
