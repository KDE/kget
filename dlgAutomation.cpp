...
DlgAutomation::DlgAutomation(QWidget * parent):QGroupBox(parent)
{
        ...
        QGridLayout *automationLayout =
                new QGridLayout(this, 5, 5, 20, KDialog::spacingHint());
        ...
        connect(cb_autoSave, SIGNAL(toggled(bool)), le_autoSave,
        ...
        connect(cb_autoDisconnect, SIGNAL(toggled(bool)), this,
                SLOT(disconnectToggled(bool)));
        connect(cb_timedDisconnect, SIGNAL(toggled(bool)), spins,
                SLOT(setEnabled(bool)));
...
{
        if (cb_autoSave->isChecked() != ksettings.b_autoSave ||
                        (uint) le_autoSave->value() != ksettings.autoSaveInterval) {
